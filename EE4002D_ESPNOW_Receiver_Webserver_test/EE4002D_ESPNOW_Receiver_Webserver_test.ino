...
// Receiver MAC Address: cc:7b:5c:e8:ea:a8
// WiFi network credentials (STATION) used to send data to WebServer (Cannot use ESPNOW: Peer to Peer)
const char* ssid = "SINGTEL-467J";
const char* password = "wxa7mfeuk2";
...
// Max number of senders
#define MAX_SENDERS 5
uint8_t knownSenders[MAX_SENDERS][6];
int senderCount = 0;
...
void OnDataRecv(const uint8_t * mac_addr, const uint8_t *incomingData, int len) { 
  ...
  for (int i = 0; i < senderCount; i++) {
    if (memcmp(mac_addr, knownSenders[i], 6) == 0) {
      known = true;
      break;
    }
  }

  // Add new sender if space is available
  if (!known && senderCount < MAX_SENDERS) {
    memcpy(knownSenders[senderCount], mac_addr, 6);
    senderCount++;
  }
  
  // Update JSON for Web Server
  board["board id"] = myData.board_id;
  ...
  Serial.printf("Data Point %u: \n", myData.readingId, len);
...
void setup() {
  ...
  // Set the Wi-Fi channel to 1 (Both receiver and sender must be on the same channel to talk)
